...
public class MazeGenerator {
    ...
    public void BackTracking(int r, int c) {
        ...
        for (int[] direction : directions) {
            int newR = r + 2 * direction[0];
            int newC = c + 2 * direction[1];
            if (newR >= 0 && newR < res.length && newC >= 0 && newC < res[0].length && res[newR][newC] != ' ') {
                res[r + direction[0]][c + direction[1]] = ' ';
                BackTracking(newR, newC);
            }
        }
    ...
    public char[][] generatePerfectMaze(int rows, int columns) {
        ...
        res = new char[2 * rows + 1][2 * columns + 1];
        for (int i = 0; i < res.length; i++) {
            for (int j = 0; j < res[i].length; j++) {
                res[i][j] = '│';
            }
        }
        res[random.nextInt(rows) * 2 + 1][0] = '⇨';
        res[random.nextInt(rows) * 2 + 1][2 * columns] = '⇨';
        BackTracking(5, 5);
        return res;
    }
