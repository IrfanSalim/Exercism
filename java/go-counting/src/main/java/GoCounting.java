...
class GoCounting {
    ...
    public Player getTerritoryOwner(int x, int y) {
        ...
        return contains(playersSurrounding, 'W', 'B') ? Player.BLACK
                : contains(playersSurrounding, 'B', 'W') ? Player.WHITE : Player.NONE;
    }
    ...
    public HashMap<Player, Set<Point>> getTerritories() {
        HashMap<Player, Set<Point>> territories = new HashMap<>(
                Map.of(Player.WHITE, new HashSet<>(), Player.BLACK, new HashSet<>(), Player.NONE, new HashSet<>()));
