...
public class ZebraPuzzle {
    ...
    private boolean solveNationalities() {
        ...
        for (int i = 0; i < numPermutations; i++) {
            ...
            if (owners.get(Nationality.Norwegian) == 0 &&
                    solveColors())
                return true;
            this.owners = null;
            nextPermutation(idx);
    ...
    private boolean solveSmokes() {
        ...
        for (int i = 0; i < numPermutations; i++) {
            this.smokes = asDict(Arrays.stream(idx)
                    ...
                    .toArray(Smoke[]::new));
            if (colors.get(Color.Yellow) == smokes.get(Smoke.Kools) &&
    ...
    private boolean solvePets() {
        ...
        for (int i = 0; i < numPermutations; i++) {
            this.pets = asDict(Arrays.stream(idx)
                    ...
                    .toArray(Pet[]::new));
            if (owners.get(Nationality.Spaniard) == pets.get(Pet.Dog) &&
